<?php
// -----
// Admin-level extra language definitions for the "Edit Orders" plugin, provided by lat9.
// Copyright (C) 2017-2021, Vinos de Frutas Tropicales.
//
define('BOX_CONFIGURATION_EDIT_ORDERS', 'Edit Orders');

define('EO_ICON_DETAILS', 'Edit this Order');
define('EO_BUTTON_ICON_DETAILS', 'icon_details.gif');
define('EO_IMAGE_BUTTON_EDIT', 'button_edit.gif');
define('EO_IMAGE_BUTTON_DETAILS', 'button_details.gif');

define('EO_ZC156_FA_ICON', '<div class="fa-stack fa-lg edit"><i class="fa fa-circle fa-stack-2x base"></i><i class="fa fa-wrench fa-stack-1x overlay" title="Edit this Order"></i></div>');
define('EO_ZC157_FA_ICON', '<i class="fa fa-wrench overlay" title="Edit this Order"></i>');

define('EO_INIT_MISSING_FILES', 'A file (%1$s) required for <em>Edit Orders</em> operation is missing.');
define('EO_INIT_MISSING_NOTIFIERS', 'One or more notifiers (%2$s), required for <em>Edit Orders</em> operation, are missing from the file %1$s.');

define('EO_INIT_VERSION_UPDATED', '<em>Edit Orders</em> was updated from v%1$s to v%2$s.');
define('EO_INIT_INSTALLED', '<em>Edit Orders</em> v%s was installed.');
