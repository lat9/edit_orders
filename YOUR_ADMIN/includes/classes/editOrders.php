    ...
    public function removeTaxFromShippingCost(&$order, $module)
    {
        if (DISPLAY_PRICE_WITH_TAX == 'true' && isset($GLOBALS[$module]) && isset($GLOBALS[$module]->tax_class) && $GLOBALS[$module]->tax_class > 0) {
            ...
            if ($country_id !== false) {
                ...
                $shipping_cost_ex = $GLOBALS['currencies']->value($order->info['shipping_cost'] / $tax_rate, false, $this->currency, $this->currency_value);
                $shipping_tax = $GLOBALS['currencies']->value($shipping_cost - $shipping_cost_ex, false, $this->currency, $this->currency_value);
